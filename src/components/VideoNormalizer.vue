...
onMounted(async () => {
  try {
    ...
    const dce = await (cameraEnhancer.value =
      CameraEnhancer.createInstance(view));
    const imageEditorView = await ImageEditorView.createInstance(
      imageEditorViewContainerRef.value as HTMLDivElement
    );
    ...
    await normalizer.initSettings(template);
    ...
    confirmTheBoundary = () => {
      ...
      for (let i = 0; i < items.length; i++) {
        if (items[i].type === EnumCapturedResultItemType.CRIT_ORIGINAL_IMAGE)
          continue;
    ...
    normalize = async () => {
      ...
      let normalizeResult = await normalizer.capture(
        image,
        "DetectDocumentBoundaries_Default"
      );
      if (normalizeResult.items[0]) {
        normalizedImageContainer.value!.append(
          (normalizeResult.items[0] as NormalizedImageResultItem).toCanvas()
        );
      }
